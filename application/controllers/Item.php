...
class Item extends CI_Controller {
	...
	function get_ajax() {
        ...
        foreach ($list as $item) {
            ...
            $row = array();
            $row[] = $no.".";
            $row[] = $item->barcode.'<br><a href="'.site_url('item/barcode_qrcode/'.$item->item_id).'" class="btn btn-default btn-xs">Generate <i class="fa fa-barcode" style="margin-left: 2px;"></i><i class="fa fa-qrcode" style="margin-left: 2px;"></i></a>';
            $row[] = $item->name;
	...
	function barcode_print($id)
	{
		...
		$this->fungsi->PDFGenerator($html, 'barcode-'.$data['row']->barcode, 'A7', 'landscape');
	}
	...
	function qrcode_print($id)
	{
		...
		$this->fungsi->PDFGenerator($html, 'qrcode-'.$data['row']->barcode, 'A7', 'potrait');
	}
